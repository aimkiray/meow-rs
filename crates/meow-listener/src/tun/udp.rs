...
async fn relay_flow(
    ...
    // A downstream read cancelled by another branch loses nothing:
    // packet reads persist their progress across cancellation (see
    // VlessUdpReader / MuxPacketConn), so the next poll resumes the
    // same datagram.
