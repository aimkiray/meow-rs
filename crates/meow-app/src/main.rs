...
/// handle is torn down without touching the stored slot. The `TunConfig`
/// compare is section-only, but that is sufficient: a same-config PUT
/// restart (e.g. fake-IP change) serializes through this lane and the
/// listener's single-core gate, so a survivor here can't silently embed
/// different resolver-era inputs.
#[cfg(feature = "listener-tun")]
