...
///   i.e. 56 B -> 72 B (at the M2 72 B per-`LruEntry`-slot cap; `LruEntry`'s
///   `.val` is the entry). On Linux (`Instant` = 8 B) the same change is
...
mod tests {
    ...
    /// 72 B per-`LruEntry`-slot cap. Per-family expiry grows the struct by one
    /// `Instant`; this test locks the cap in so a future field addition can't
    /// silently breach it. See the struct doc for the before/after byte counts.
    #[test]
