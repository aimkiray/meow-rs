...
| C4 | `body_none_cipher_is_passthrough` | `Security::None` writes framed raw bytes (`[len(2)][payload]` — OPT_STANDARD still chunk-streams); round-trip asserts no encryption overhead. |
...
| C6 | `body_nonce_counter_increments_per_record` | Three consecutive records encoded; assert each record's nonce is `counter_be16 \|\| body_iv[2..12]` with counter 0, 1, 2 respectively. Nonces must be distinct. |
...
| D7 | `parse_vmess_cipher_none_accepted` | `cipher: none`. Parses without error; body cipher is `RecordCipher::None` under `Security::None`. |
