...
| `proxy` | string | no | `""` | Route this provider's `http` fetches through the named proxy or group, resolved against the live route map at fetch time (issue #625). `DIRECT` or absent fetches directly; an unresolvable name fails the fetch (never silently direct). Meaningless on `file` providers (warns). |
